    ...
    <c:param name="content">
        ...
        <table id="memo_list">
            <tbody>
                ...
                <c:forEach var="memo" items="${memos}" varStatus="status">
                    <fmt:parseDate value="${memo.reportDate}" pattern="yyyy-MM-dd"
                        var="reportDay" type="date" />


                    <tr class="row${status.count % 2}">

                        <td class="memo_date"><fmt:formatDate value='${reportDay}'
                                pattern='yyyy-MM-dd' /></td>
                        <td class="memo_title">${memo.title}</td>
                        <td class="memo_content">${memo.content}</td>
                        <td class="memo_action"><a
                            href="<c:url value='?action=${actMem}&command=${commShow}&id=${memo.id}' />">詳細を見る</a></td>
                    </tr>
                </c:forEach>
            </tbody>
        </table>
        ...
        <div id="pagination">
            ...
            <c:forEach var="i" begin="1"
                end="${((memos_count - 1) / maxRow) + 1}" step="1">
                <c:choose>
                    <c:when test="${i == page}">
                        <c:out value="${i}" />&nbsp;
                    </c:when>
                    <c:otherwise>
                        <a
                            href="<c:url value='?action=${actMem}&command=${commIdx}&page=${i}' />"><c:out
                                value="${i}" /></a>&nbsp;
                    </c:otherwise>
                </c:choose>
            </c:forEach>
        ...
        <p>
            <a href="<c:url value='?action=${actMem}&command=${commNew}' />">新規メモの登録</a>
        </p>
